...
public class Player extends SWActor {
	...
	public void describeScene() {
		...
		//if (location == BenKenobi.getBenKenobi(messageRenderer, world, moves, getForcePoints()).getWorld().getEntityManager().whereIs(ben)) {
		//if (location == SWWorld.initializeWorld(messageRenderer).ben.getWorld().getEntityManager().whereIs(ben)) {
		if (location.getSymbol() == (char) 'B') {
			this.setForcePoints(100000);
		}
