...
public class SWWorld extends World {
	...
	public void initializeWorld(MessageRenderer iface) {
		...
		BenKenobi ben = BenKenobi.getBenKenobi(iface, this, patrolmoves, 200);
		ben.setSymbol("B");
		...
		// Initialize Jawa Sandcrawler first, so that actors get immediate teleportation once they move to the Sandcrawler.
		Sandcrawler SandC = new Sandcrawler(50000000, iface, this);
		SandC.setSymbol("S");
		loc = myGrid.getLocationByCoordinates(4,4);
		entityManager.setLocation(SandC, loc);
				
		
		// Initialize a 2x2 grid as space inside sandcrawler
		for (int col=0; col<2; col++) {
			for (int row=0; row<2; row++) {
				//VoidSpace voidspace = new VoidSpace(iface);
				loc = myGrid.getLocationByCoordinates(col, row);
				//entityManager.setLocation(voidspace, loc);
			}
		}
		...
		// Droid R2D2
		Droid R2D2 = new Droid(50, "R2D2", iface, this, luke);
		R2D2.setSymbol("R");
		...
		// An HP Regeneration Pill
		HPPill pill1 = new HPPill(iface);
		loc = myGrid.getLocationByCoordinates(4, 9);
		entityManager.setLocation(pill1, loc);
		
		// Another HP Regeneration Pill
		HPPill pill2 = new HPPill(iface);
		loc = myGrid.getLocationByCoordinates(0, 7);
		entityManager.setLocation(pill2, loc);
